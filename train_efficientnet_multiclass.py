...
EFFNET_WEIGHTS_PATH = "best_efficientnet_multiclass.pth"
LABELS = ['healthy', 'sb', 'dead', 'missing']

class ZebrafishDataset(Dataset):
    def __init__(self, csv_file=None, image_dirs=None, transform=None, data=None):
        if csv_file:
            self.data = pd.read_csv(csv_file)
            self.data = self.data[self.data['disease'].isin(LABELS)].reset_index(drop=True)
    ...
    def __getitem__(self, idx):
        ...
        if image is None or label is None:
            return None
        return image, LABELS.index(label)
